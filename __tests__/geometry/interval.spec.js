...
describe('interval', () => {
  ...
  test('column chart for x, y, fill channels', () => {
    plot({
      ...
      scales: {
        x: createBand({
          domain: ['a', 'b', 'c'], // 隐含的 x1 的值是 x 通过 bandScale 映射得到
          range: [0, 1],
          padding: 0.2,
        }),
      },
  ...
  test('donut for ring', () => { // 甜甜圈
    plot({
      ...
      scales: {
        x: createBand({ domain: [0], range: [0, 1], padding: 0 }), // 666
      },
      channels: {
        fill: ['#5B8FF9', '#5AD8A6', '#5D7092'],
        x: [0, 0, 0], // 666 // x 轴拉满
  ...
  test('pie for transpose, polar', () => { // 派
    plot({
      geometry: interval,
      index: [0, 1, 2],
      styles: {
