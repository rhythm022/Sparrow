...
export function ticksLeft(renderer, ticks, { tickLength, fontSize }) {
  for (const { x, y, text } of ticks) {
    ...
    renderer.text({
      text, fontSize, x: x2, y, textAnchor: 'end', dy: '0.5em', dx: '-0.5em', class: 'text',
    });
  }
}

export function ticksCircular(renderer, ticks, { tickLength, fontSize, center }) { // 666 对角度的处理
  for (const { x, y, text } of unique(ticks, (d) => d.x, (d) => d.y)) {
    const { tickRotation, textRotation } = rotationOf(center, [x, y]);
    const [x2, y2] = [0, tickLength];// 相对的
    ...
    renderer.rotate(degree(textRotation));// 相对的
